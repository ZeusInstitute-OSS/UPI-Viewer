    ...
    class SMSService : Service(), TextToSpeech.OnInitListener {
        ...
        private fun announceMessage(message: String) {
            ...
            val params = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                Bundle().apply {
                    val sharedPref = getSharedPreferences("com.zeusinstitute.upiapp.preferences", MODE_PRIVATE)
                    val selectedVoiceName = sharedPref.getString("selected_voice", null)
                    if (selectedVoiceName != null) {
                        putString(TextToSpeech.Engine.EXTRA_AVAILABLE_VOICES, selectedVoiceName) // Correct way to set value in Bundle
                    }
                }} else {
                HashMap<String, String>().apply {
                    ...
                    val selectedVoiceName = sharedPref.getString("selected_voice", null)
                    if (selectedVoiceName != null) {
                        put(TextToSpeech.Engine.EXTRA_AVAILABLE_VOICES, selectedVoiceName) // Correct way to set value in HashMap
                    }
                }
            }

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                tts?.speak(message, TextToSpeech.QUEUE_ADD, params as Bundle, "UPI_CREDIT")
            } else {
                @Suppress("DEPRECATION")
                tts?.speak(message, TextToSpeech.QUEUE_ADD, params as HashMap<String, String>)
            }
        }
        ...
        override fun onInit(status: Int) {
            if (status == TextToSpeech.SUCCESS) {
                ...
                val intent = Intent("com.zeusinstitute.upiapp.TTS_READY")

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    val voices = tts?.voices?.filter { it.locale == Locale.ENGLISH } ?: emptyList()
                    val voiceNames = voices.map { it.name }
                    Log.d("SMSService", "Voices: ${ArrayList(voiceNames)}")
                    intent.putStringArrayListExtra("voices", ArrayList(voiceNames))
                } else {
                    // Indicate that no voice selection is available
                    intent.putExtra("voices", "Default")
                }
